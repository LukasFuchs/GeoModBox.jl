...
function TdepViscosity!(D,b,c,y)
    
    # Non-dimensional - assuming η is scaled by P.η₀
    @. D.ηc     =   exp(-b*D.T + c*y.c2d)

    # --- Extended Centroids; required to get ηᵥ
...
p2 = heatmap(x.c,y.c,D.T',
    ...
    heatmap!(x.c,y.c,log10.(D.ηc'),
            ...
            layout=(1,2),subplot=2)
if save_fig == 1
    savefig(k,string("./examples/StokesEquation/2D/Results/Blankenbach_var_eta_iterations_",@sprintf("%.2e",P.Ra),
            "_",NC.x,"_",NC.y,
            "_",Ini.T,".png"))
    savefig(p2,string("./examples/StokesEquation/2D/Results/Blankenbach_var_eta_Final_Stage_",@sprintf("%.2e",P.Ra),
...
plot!(q2,Time[1:find],B.Vrms[B.Nr].*ones(find,1),
            ...
            layout=(2,1),subplot=2)
if save_fig == 1
    savefig(q2,string("./examples/StokesEquation/2D/Results/Blankenbach_var_eta_TimeSeries_",@sprintf("%.2e",P.Ra),
...
scatter!(q3,(B.Tmax[B.Nr],-(1-B.ymax[B.Nr])),
            ...
            markercolor=:black,label=false)
if save_fig == 1
    savefig(q3,string("./examples/StokesEquation/2D/Results/Blankenbach_var_eta_Profiles_",@sprintf("%.2e",P.Ra),
                        "_",NC.x,"_",NC.y,"_",Ini.T,".png"))
elseif save_fig == 0
    display(q3)
end
