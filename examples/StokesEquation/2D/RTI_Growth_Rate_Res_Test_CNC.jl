...
function RTI_GrowthRate()
    ...
    # Summary errors:
    # Dimensions:
    # averaging scheme × marker distribution × viscosity ratio × marker density
    ε_summary = fill(
        NaN,
        length(avg_list),
        length(addnoise),
    ...
    for ia in eachindex(avg_list)
            ...
            for o in eachindex(ηᵣ) # Loop over viscosity ratio
                ...
                for l in eachindex(delfac) # Loop over perturbation amplitude
                    ...
                    for k in eachindex(nm) # Loop over marker numbers
                        ...
                        if k == 1 && n == 1 && o == 1
                            scatter!(q,(nm[k]*nm[k],PP.ε[1]),
                                        ms=ms[1],markershape=:circle,
                                        label=string(-δA," [m]"),color=mc[l],
                                        xlabel="Markers per cell",ylabel="ε [ % ]",
        ...
        # ------------------------------------------------------------------ #
        if save_fig == 1
        savefig(q,string("./examples/StokesEquation/2D/Results/RTI_Growth_Rate_Res_Test_const_NC_",
    ...
    if save_fig == 1
        savefig(
            ...
            string(
                ...
                "RTI_Growth_Rate_Averaging_Summary_const_NC_",
